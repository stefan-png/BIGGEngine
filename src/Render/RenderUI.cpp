...
namespace RenderUI {
namespace {
    ImGuiKey KeyEnumToImGuiKey(KeyEnum key) {
        switch (key)
        {
    ...
    bool handleWindowCreateEvent(WindowCreateEvent) {
        ...
        data = BX_NEW(Context::getAllocator(), RenderUIData);

        io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;
        io.DisplaySize = Context::getWindowFramebufferSize();
        return false;
    }

    bool handleWindowSizeEvent(WindowSizeEvent) {
        ImGuiIO &io = ImGui::GetIO();
        io.DisplaySize = Context::getWindowFramebufferSize();
    ...
    bool handleEarlyUpdateEvent(UpdateEvent e) {
        BIGG_PROFILE_UI_FUNCTION;
        ImGuiIO &io = ImGui::GetIO();

        io.DeltaTime = e.m_delta;
        ImGui::NewFrame();
        return false;
    }
    ...
    bool handleLateUpdateEvent(UpdateEvent) {
        ...
        BIGG_ASSERT(data != nullptr, "data isn't initialized!");
        ...
        bgfx::setViewMode(viewID, bgfx::ViewMode::Sequential);  // Sequential means things are drawn same order they are submitted to bgfx
        ...
        for (int32_t ii = 0, num = drawData->CmdListsCount; ii < num; ++ii) {
            ...
            if (!(numVertices == bgfx::getAvailTransientVertexBuffer(numVertices, data->m_layout)
            ...
            bgfx::allocTransientVertexBuffer(&tvb, numVertices, data->m_layout);
            ...
            for (const ImDrawCmd *cmd = drawList->CmdBuffer.begin(), *cmdEnd = drawList->CmdBuffer.end();
                ...
                if (cmd->UserCallback) {
                    ...
                    bgfx::TextureHandle th = data->m_texture;
                    bgfx::ProgramHandle program = data->m_program;
...
#define IMGUI_FLAGS_ALPHA_BLEND 0x01
                    ...
                    if (NULL != cmd->TextureId) {
                        ...
                        if (0 != texture.s.mip) {
                            ...
                            bgfx::setUniform(data->u_imageLodEnabled, lodEnabled);
                            program = data->m_imageProgram;
                        }
                    } else {
                    ...
                    if (clipRect.x < fb_width
                        ...
                        encoder->setTexture(0, data->s_tex, th);
    ...
    void init() {
        ...
        io.SetClipboardTextFn = [](void*, const char* string){
            Context::setClipboardString(string);
        };
        io.GetClipboardTextFn = [](void*) -> const char*{
            return Context::getClipboardString().c_str();
        };

        // TODO cursors
        Events::subscribe<WindowCreateEvent>(g_renderUIBeginPriority,  handleWindowCreateEvent  );
        Events::subscribe<WindowSizeEvent>(g_renderUIBeginPriority,    handleWindowSizeEvent    );
        Events::subscribe<WindowFocusEvent>(g_renderUIBeginPriority,   handleWindowFocusEvent   );
        Events::subscribe<MouseEnterEvent>(g_renderUIBeginPriority,    handleMouseEnterEvent    );
        Events::subscribe<MousePositionEvent>(g_renderUIBeginPriority, handleMousePositionEvent );
        Events::subscribe<MouseButtonEvent>(g_renderUIBeginPriority,   handleMouseButtonEvent   );
        Events::subscribe<ScrollEvent>(g_renderUIBeginPriority,        handleScrollEvent        );
        Events::subscribe<KeyEvent>(g_renderUIBeginPriority,           handleKeyEvent           );
        Events::subscribe<CharEvent>(g_renderUIBeginPriority,          handleCharEvent          );
        Events::subscribe<UpdateEvent>(g_renderUIBeginPriority,        handleEarlyUpdateEvent   );
        // TODO add monitor callback for "docking" branch of imgui

        Events::subscribe<UpdateEvent>(g_renderUIEndPriority,          handleLateUpdateEvent    );

    }
    void shutdown() {
        BIGG_PROFILE_SHUTDOWN_FUNCTION;
        ImGui::DestroyContext();
        BX_DELETE(Context::getAllocator(), data);
    }
} // namespace RenderUI
} // namespace BIGGEngine